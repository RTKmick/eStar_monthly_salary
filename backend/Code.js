...
// ==========================================
// 🔥 1. 取得「薪資總覽」資料 (修正版：合併平日與假日列，並讀取 G/H 欄)
// ==========================================
function getSalaryDashboardData() {
  ...
  // 讀取 A3 到 I欄 (全範圍)
  // 欄位索引: A=0, B=1, C=2, D=3, E=4, F=5, G=6, H=7, I=8
  const data = sheet.getRange(3, 1, lastRow - 2, 9).getDisplayValues(); 
  let result = [];
  
  for (let i = 0; i < data.length; i++) {
    let row = data[i];
    let name = row[0]; // A欄: 姓名

    // ★ 關鍵邏輯：只處理「有名字」的那一列 (通常是平日列)
    if (name && name !== "" && !name.includes("總計")) {
      
      // 1. 取得平日獎金 (F欄)
      // 移除 $ 和 , 符號轉成數字
      let bonus1 = parseFloat(String(row[5]).replace(/[$,]/g, '')) || 0; 
      
      // 2. 嘗試取得下一列 (假日列) 的獎金
      // 如果下一列存在，且名字是空的，代表它是同一個人的假日資料
      let bonus2 = 0;
      if (i + 1 < data.length) {
      ...
      // 3. 讀取 G 欄 (倍數) 與 H 欄 (職務獎金)
      let multiplier = row[6]; // G欄 (例如 "X1.0" 或 "1")
      let jobBonus = row[7];   // H欄 (職務獎金)
      let totalSalary = row[8]; // I欄 (總薪資)

      result.push({
        rowIndex: i + 3,    // 紀錄列號 (方便寫回 G 欄)
        name: name,
        hours: row[3],      // D欄: 當月總工時
        meal: row[4],       // E欄: 伙食津貼 (通常只有第一列有值)
        bonus: bonus1 + bonus2, // ★ 修正：將兩列的獎金加總顯示
        multiplier: multiplier, // G欄
        jobBonus: jobBonus,     // H欄
        salary: totalSalary     // I欄
      });
    }
  }
  return result;
